...
export class DashboardComponent implements OnInit {
  ...
  private loadAggregatedData(): void {
    ...
    this.municipalityService.getMunicipalities().subscribe({
      next: (municipalities) => {
        ...
        // Load top municipalities data from waste collections
        this.wasteCollectionService.getWasteCollections().subscribe({
          next: (collections) => {
            const collectionsArray = Array.isArray(collections) ? collections : [];
            
            // Calculate collections per municipality
            const municipalityStats = new Map<string, any>();
            collectionsArray.forEach((c: any) => {
              const mId = c.municipalityId?.toString();
              if (mId) {
                if (!municipalityStats.has(mId)) {
                  municipalityStats.set(mId, {
                    municipalityId: mId,
                    collections: 0,
                    waste: 0,
                    points: 0
                ...
                const stats = municipalityStats.get(mId);
                stats.collections++;
                stats.waste += c.weight || 0;
                stats.points += c.points || 0;
              }
            });
            
            // Map to top municipalities
            this.topMunicipalities = filteredMunicipalities
              .map(m => {
                const stats = municipalityStats.get(m.id) || { collections: 0, waste: 0, points: 0 };
                return {
                  name: m.name,
                  collections: stats.collections,
                  waste: Math.round(stats.waste * 10) / 10,
                  citizens: m.population || 0,
                  participation: m.population ? Math.round((stats.collections / m.population) * 100) : 0
                };
              })
            ...
            filteredMunicipalities.forEach(m => {
              ...
              data.municipalities++;
              const mStats = municipalityStats.get(m.id) || { collections: 0, waste: 0 };
              data.collections += mStats.collections;
              data.waste += mStats.waste;
            });
            this.regionalPerformance = Array.from(regionMap.values());
          },
          error: (error) => {
            console.error('Error loading collections for top municipalities:', error);
            this.topMunicipalities = [];
            this.regionalPerformance = [];
          }
        });
  ...
  private generateTopCollectionPoints(collections: any[]): void {
    ...
    this.topCollectionPoints = Array.from(cpMap.values())
      ...
      .map(cp => ({
        ...cp,
        participation: 0 // Will be calculated from real data
      }));
  }
